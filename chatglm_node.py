...
def getConfigData():
    ...
    config_path = os.path.join(os.path.abspath(dir_node), "chatglm_config.json")
...
def checkPropValue(obj, key, not_include=[]):
    def checkVal(v):
        return v is None or v.strip() == "" or v in not_include
...
class ChatGLM4InstructNode_Copy:
    @classmethod
...
class ChatGLM4InstructMediaNode_Copy:
    @classmethod
    def INPUT_TYPES(self):
        return {
            "optional": {
